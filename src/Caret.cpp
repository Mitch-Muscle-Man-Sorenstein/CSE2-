...
void ActCaret08(CARET *crt)
{
	...
	if (gMC_Costume != MCC_Normal)
	{
		crt->rect.top += 160 + ((gMC_Costume - 1) * 16);
		crt->rect.bottom += 160 + ((gMC_Costume - 1) * 16);
	}
}
...
void PutCaret(int fx, int fy)
{
	...
	for (i = 0; i < CARET_MAX; ++i)
	{
		if (gCrt[i].cond & 0x80)
		{
			PutBitmap3(
				&grcGame,
				SubpixelToScreen(gCrt[i].x - gCrt[i].view_left) - SubpixelToScreen(fx),
				SubpixelToScreen(gCrt[i].y - gCrt[i].view_top) - SubpixelToScreen(fy),
				&gCrt[i].rect,
				SURFACE_ID_CARET);
		}
	}
